...
const Section = ({title,description,isVisible,setIsVisible})=>{
  ...
  <div className="border-2 border-black p-2 m-6">
    <div className="flex ">
      ...
      {isVisible?<button className="px-2" onClick={()=>setIsVisible(false)}><FontAwesomeIcon icon={faArrowUp} /></button>:<button className="px-2" onClick={()=>setIsVisible(true)}><FontAwesomeIcon icon={faArrowDown} /></button>}
    </div>
    {isVisible&&<h2 className="text-xl">{description}</h2>}
  </div>
  )
}
